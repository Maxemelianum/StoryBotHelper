...
wb = load_workbook('-COPY - Copy.xlsx')
vendor = wb['Лист7']
...
def do_delete_vendor(update: Update, context):
    ...
    for i in range(17, 100):
        ...
        if vendor.cell(column=3, row=i).value == update.message.text:
            vendor.cell(column=3, row=i).value = None
            update.message.reply_text(text='Готово!')
            break
